...
_MODEL_CFG_KEYS = [
    key for key in _FALLBACK_MODEL_CFG
    if key in inspect.signature(PCVRHyFormer.__init__).parameters
]
_IGNORED_MODEL_CFG_KEYS = [
    key for key in _FALLBACK_MODEL_CFG
    if key not in inspect.signature(PCVRHyFormer.__init__).parameters
]
if _IGNORED_MODEL_CFG_KEYS:
    logging.info(
        "Ignoring model cfg keys unsupported by this model.py: "
        f"{_IGNORED_MODEL_CFG_KEYS}")
...
def _batch_to_model_input(
    ...
    global_last_seq_time_bucket = device_batch.get('global_last_seq_time_bucket')

    for domain in seq_domains:
        seq_data[domain] = device_batch[domain]
        seq_lens[domain] = device_batch[f'{domain}_len']
        B, _, L = device_batch[domain].shape
        time_bucket = device_batch.get(
            f'{domain}_time_bucket',
            torch.zeros(B, L, dtype=torch.long, device=device))
        if NUM_TIME_BUCKETS > 0:
            time_bucket = time_bucket.long().clamp_(min=0, max=NUM_TIME_BUCKETS - 1)
        seq_time_buckets[domain] = time_bucket

    if global_last_seq_time_bucket is None:
        batch_size = device_batch['user_int_feats'].shape[0]
        global_last_seq_time_bucket = torch.zeros(
            batch_size, dtype=torch.long, device=device)
    elif NUM_TIME_BUCKETS > 0:
        global_last_seq_time_bucket = global_last_seq_time_bucket.long().clamp_(
            min=0, max=NUM_TIME_BUCKETS - 1)
    ...
    return ModelInput(
        ...
        global_last_seq_time_bucket=global_last_seq_time_bucket,
    )
...
def main() -> None:
    ...
    use_fid16_te = bool(train_config.get('use_fid16_te', False))
    fid16_te_stats_path = train_config.get('fid16_te_stats_path', None)
    if use_fid16_te:
        if fid16_te_stats_path:
            local_candidate = os.path.join(
                model_dir, os.path.basename(fid16_te_stats_path))
            if os.path.exists(local_candidate):
                fid16_te_stats_path = local_candidate
        else:
            fid16_te_stats_path = os.path.join(model_dir, FID16_TE_STATS_FILENAME)
        if not fid16_te_stats_path or not os.path.exists(fid16_te_stats_path):
            raise FileNotFoundError(
                "Checkpoint was trained with use_fid16_te=True, but "
                f"fid16 TE stats were not found: {fid16_te_stats_path!r}")
        logging.info(f"Using fid16 TE stats: {fid16_te_stats_path}")
