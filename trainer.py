...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        loss_log_every_n_steps: int = 100,
        ...
        self.train_model: nn.Module = model
        ...
        # fid16 target-encoding stats are needed by infer.py to reproduce the
        # same synthetic item-int buckets used during training.
        self.fid16_te_stats_path: Optional[str] = fid16_te_stats_path
        ...
        self.loss_log_every_n_steps: int = loss_log_every_n_steps
        self.warmup_ratio: float = warmup_ratio
        self.min_lr_ratio: float = min_lr_ratio
        self.lr_scheduler: str = lr_scheduler
        self.use_amp_bfloat16: bool = (
            use_amp_bfloat16
        ...
        self.compile_active: bool = False
        self.train_config: Optional[Dict[str, Any]] = train_config
        self.total_train_steps = max(1, len(self.train_loader) * self.num_epochs)
        self.warmup_steps = int(self.total_train_steps * self.warmup_ratio)
        self.dense_base_lr = lr
        self.sparse_base_lr = sparse_lr

        if self.use_torch_compile and hasattr(torch, 'compile'):
            try:
                self.train_model = torch.compile(self.model, mode='reduce-overhead')
                self.compile_active = True
                logging.info("torch.compile enabled for training forward pass")
        ...
        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     ...
                     f"lr_scheduler={lr_scheduler}, warmup_steps={self.warmup_steps}, "
                     f"total_train_steps={self.total_train_steps}, "
                     f"min_lr_ratio={min_lr_ratio}, amp_bfloat16={self.use_amp_bfloat16}")
    ...
    def _write_sidecar_files(self, ckpt_dir: str) -> None:
        ...
        - ``fid16_te_stats.json`` (copied from ``self.fid16_te_stats_path``
          when set): bucket maps for the synthetic fid16 item-int features.
        - ``train_config.json`` (serialized from ``self.train_config``):
          full set of training-time hyperparameters. When ``ns_groups.json``
          or ``fid16_te_stats.json`` is copied into ``ckpt_dir``, the
          corresponding config path is
        ...
        fid16_te_stats_copied = False
        if self.fid16_te_stats_path and os.path.exists(self.fid16_te_stats_path):
            shutil.copy2(self.fid16_te_stats_path, ckpt_dir)
            fid16_te_stats_copied = True

        if self.train_config:
            import json
            cfg_to_dump = self.train_config
            if ns_groups_copied or fid16_te_stats_copied:
            ...
            if ns_groups_copied:
                ...
                cfg_to_dump['ns_groups_json'] = os.path.basename(
                    self.ns_groups_path)
            if fid16_te_stats_copied:
                cfg_to_dump['fid16_te_stats_path'] = os.path.basename(
                    self.fid16_te_stats_path)
    ...
    def _lr_multiplier(self, step: int) -> float:
        """Return the step-level LR multiplier for warmup + decay."""
        if self.lr_scheduler == 'none':
            return 1.0
        if self.warmup_steps > 0 and step <= self.warmup_steps:
            return max(step / self.warmup_steps, 1e-8)
    ...
    def _update_learning_rates(self, step: int) -> None:
        """Apply the current LR multiplier to dense and sparse optimizers."""
        mult = self._lr_multiplier(step)
        for group in self.dense_optimizer.param_groups:
            group['lr'] = self.dense_base_lr * mult
        if self.sparse_optimizer is not None:
            for group in self.sparse_optimizer.param_groups:
                group['lr'] = self.sparse_base_lr * mult

    def _current_lrs(self) -> Tuple[float, float]:
        dense_lr = float(self.dense_optimizer.param_groups[0]['lr'])
        sparse_lr = 0.0
        if self.sparse_optimizer is not None:
            sparse_lr = float(self.sparse_optimizer.param_groups[0]['lr'])
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for step, batch in train_pbar:
                total_step += 1
                self._update_learning_rates(total_step)
                loss = self._train_step(batch)
                loss_sum += loss

                if self.writer and total_step % self.loss_log_every_n_steps == 0:
                    self.writer.add_scalar('Loss/train', loss, total_step)
                    dense_lr, sparse_lr = self._current_lrs()
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        return ModelInput(
            ...
            global_last_seq_time_bucket=device_batch.get(
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        try:
            with torch.autocast(
                ...
                enabled=self.use_amp_bfloat16,
            ):
                logits = self.train_model(model_input)  # (B, 1)
        except Exception as exc:
            if not self.compile_active:
                raise
            logging.warning(f"Compiled forward failed, falling back to eager mode: {exc}")
            self.train_model = self.model
            self.compile_active = False
            with torch.autocast(
                device_type='cuda',
