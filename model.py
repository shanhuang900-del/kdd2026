...
class ModelInput(NamedTuple):
    ...
    global_last_seq_time_bucket: torch.Tensor  # [B], 0 means no valid sequence event
...
class DINAttentionPooling(nn.Module):
    """DIN-style target-aware attention pooling over one behavior sequence."""
    ...
    def forward(
        ...
        """Pool history tokens with target-aware DIN attention.

        Args:
            target: (B, D), target/context vector.
            history: (B, L, D), sequence tokens.
        ...
        scores = self.scorer(din_input).squeeze(-1)  # (B, L)
        ...
        return torch.bmm(weights.unsqueeze(1), history).squeeze(1)  # (B, D)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_global_seq_recency: bool = True,
    ) -> None:
        super().__init__()
        ...
        self.use_global_seq_recency = use_global_seq_recency
        ...
        # A single global "latest sequence event -> current label time" bucket,
        # injected immediately before the final classifier so it cannot be
        # diluted by sequence attention/pooling.
        if self.use_global_seq_recency and num_time_buckets > 0:
            self.global_seq_recency_embedding = nn.Embedding(
                num_time_buckets, d_model, padding_idx=0)
            self.global_seq_recency_fusion = nn.Sequential(
        ...
        # DIN-style target-aware pooling over sequence tokens. The target/query
        # is derived from the current item/atom token.
        if self.use_din:
            din_hidden_dim = max(2, din_hidden_dim)
            if din_hidden_dim % 2 != 0:
                din_hidden_dim += 1
            self.din_poolers = nn.ModuleList([
                DINAttentionPooling(d_model, din_hidden_dim, din_dropout)
                for _ in range(self.num_sequences)
            ])
            self.din_fusion = nn.Sequential(
                nn.Linear((self.num_sequences + 1) * d_model, d_model),
                nn.LayerNorm(d_model),
                nn.SiLU(),
                nn.Dropout(dropout_rate),
            )
    ...
    def _init_params(self) -> None:
        ...
        if self.use_global_seq_recency and self.num_time_buckets > 0:
            nn.init.xavier_normal_(self.global_seq_recency_embedding.weight.data)
            self.global_seq_recency_embedding.weight.data[0, :] = 0
    ...
    def reinit_high_cardinality_params(
        ...
        # Global sequence recency embedding is a protected low-cardinality
        # temporal feature, same as the per-token time embedding.
        if self.use_global_seq_recency and self.num_time_buckets > 0:
            skip_count += 1
    ...
    def _apply_din_fusion(
        self,
        output: torch.Tensor,
        din_target: torch.Tensor,
        seq_tokens_list: list,
        seq_masks_list: list,
    ) -> torch.Tensor:
        """Fuse HyFormer output with DIN-pooled sequence interests."""
        if not self.use_din:
            return output

        din_vectors = [
            pooler(din_target, seq_tokens, seq_mask)
            for pooler, seq_tokens, seq_mask in zip(
                self.din_poolers, seq_tokens_list, seq_masks_list)
        ]
        din_input = torch.cat([output] + din_vectors, dim=-1)
        return self.din_fusion(din_input)

    def _apply_global_seq_recency(
        self,
        output: torch.Tensor,
        bucket_ids: torch.Tensor,
    ) -> torch.Tensor:
        """Fuse one global latest-sequence recency bucket before classification."""
        if not self.use_global_seq_recency or self.num_time_buckets <= 0:
            return output
        recency = self.global_seq_recency_embedding(bucket_ids.long())
        return self.global_seq_recency_fusion(torch.cat([output, recency], dim=-1))
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        atom_target = item_ns.mean(dim=1)  # (B, D), DIN target/query
        ...
        output = self._apply_din_fusion(
            output, atom_target, seq_tokens_list, seq_masks_list)
        output = self._apply_global_seq_recency(
            output, inputs.global_last_seq_time_bucket)
