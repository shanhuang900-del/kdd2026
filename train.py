...
from dataset import (
    FeatureSchema,
    get_pcvr_data,
    build_fid16_te_stats,
    NUM_TIME_BUCKETS,
    FID16_TE_STATS_FILENAME,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--loss_log_every_n_steps', type=int, default=100,
                        help='Write training loss to TensorBoard every N steps')
    parser.add_argument('--warmup_ratio', type=float, default=0.05,
                        help='Fraction of total train steps used for LR warmup')
    parser.add_argument('--min_lr_ratio', type=float, default=0.1,
                        help='Final LR ratio relative to base LR after decay')
    parser.add_argument('--lr_scheduler', type=str, default='cosine',
                        choices=['none', 'linear', 'cosine'],
                        help='Step-level LR scheduler after warmup')
    parser.add_argument('--use_amp_bfloat16', action='store_true', default=True,
                        help='Enable torch autocast with bfloat16 on CUDA (default on)')
    parser.add_argument('--no_amp_bfloat16', dest='use_amp_bfloat16', action='store_false',
                        help='Disable bfloat16 AMP')
    parser.add_argument('--use_torch_compile', action='store_true', default=True,
                        help='Compile the training forward graph with torch.compile (default on)')
    ...
    parser.add_argument('--use_fid16_te', action='store_true', default=True,
                        help='Enable smoothed target-encoding buckets for item_int_feats_16 '
                             '(default on)')
    parser.add_argument('--no_fid16_te', dest='use_fid16_te', action='store_false',
                        help='Disable fid16 target-encoding bucket features')
    parser.add_argument('--fid16_te_stats_path', type=str, default=None,
                        help='Existing/output fid16 TE stats JSON path. If omitted, '
                             '<ckpt_dir>/fid16_te_stats.json is created from train Row Groups.')
    parser.add_argument('--fid16_te_smoothing', type=float, default=50.0,
                        help='Additive smoothing strength for fid16 smoothed CVR prior')
    ...
    parser.add_argument('--use_global_seq_recency', action='store_true', default=True,
                        help='Inject one global latest-sequence recency bucket directly '
                             'before the final classifier (default on)')
    parser.add_argument('--no_global_seq_recency', dest='use_global_seq_recency',
                        action='store_false',
                        help='Disable the global latest-sequence recency classifier input')
    ...
    parser.add_argument('--use_din', action='store_true', default=True,
                        help='Enable DIN-style target-aware pooling over sequence tokens (default on)')
    parser.add_argument('--no_din', dest='use_din', action='store_false',
                        help='Disable DIN-style target-aware pooling')
    parser.add_argument('--din_hidden_dim', type=int, default=64,
                        help='Hidden dimension of the DIN attention scorer')
    parser.add_argument('--din_dropout', type=float, default=0.01,
                        help='Dropout rate inside the DIN attention scorer')
...
def main() -> None:
    ...
    if args.use_fid16_te:
        if not args.fid16_te_stats_path:
            args.fid16_te_stats_path = os.path.join(
                args.ckpt_dir, FID16_TE_STATS_FILENAME)
        build_fid16_te_stats(
            data_dir=args.data_dir,
            output_path=args.fid16_te_stats_path,
            valid_ratio=args.valid_ratio,
            train_ratio=args.train_ratio,
            smoothing=args.fid16_te_smoothing,
        )
    else:
        args.fid16_te_stats_path = None
    ...
    train_loader, valid_loader, pcvr_dataset = get_pcvr_data(
        ...
        fid16_te_stats_path=args.fid16_te_stats_path,
        use_fid16_te=args.use_fid16_te,
    )
    ...
    model_args = {
        ...
        "use_global_seq_recency": args.use_global_seq_recency,
        ...
        "din_hidden_dim": args.din_hidden_dim,
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        loss_log_every_n_steps=args.loss_log_every_n_steps,
        warmup_ratio=args.warmup_ratio,
        min_lr_ratio=args.min_lr_ratio,
        lr_scheduler=args.lr_scheduler,
        use_amp_bfloat16=args.use_amp_bfloat16,
